...
export const Button: React.FC<ButtonProps> = ({ children, variant = 'primary', className = '', ...props }) => {
  const baseStyle = "px-4 py-2 rounded border transition-all duration-300 font-serif tracking-wide disabled:opacity-50 disabled:cursor-not-allowed";
  
  const variants = {
    primary: "bg-stone-800 border-amber-700 text-amber-500 hover:bg-stone-700 hover:text-amber-400 hover:border-amber-500 shadow-[0_0_10px_rgba(180,83,9,0.2)]",
...
export const ProgressBar: React.FC<{ value: number; max: number; color: string; label: string }> = ({ value, max, color, label }) => {
  ...
  return (
    <div className="w-full mb-2">
      <div className="flex justify-between text-xs text-stone-400 mb-1 font-serif">
        <span>{label}</span>
        <span>{value} / {max}</span>
      </div>
...
export const Divider: React.FC = () => (
  <div className="flex items-center my-4 opacity-50">
    ...
    <div className="mx-2 text-amber-900 text-xs">✦</div>
    <div className="flex-grow h-px bg-gradient-to-r from-transparent via-amber-800 to-transparent"></div>
  </div>
);
